...
def decrypt(cipher_text, initial_roundkey):
    generate_roundkeys(initial_roundkey)
    decipher_text_chunks.clear()
    #convert recieved encrypted_response.encode('utf-8') to hex
    cipher_text = BitVector(hexstring=cipher_text.decode('utf-8'))

    chunk_count = int(len(cipher_text) / 128)
    ...
    return decipher_text.get_hex_string_from_bitvector()

def generate_roundkeys(initial_roundkey):
    initial_roundkey=str(initial_roundkey)
    ...
    #for loop to generate all the roundkeys
    for i in range(10):
        roundkeys.append(generate_roundkey(roundkeys[i], i+1))

def encrypt(plain_text, initial_roundkey):
    plain_text_hex = plain_text.encode("utf-8").hex()
    cipher_text_chunks.clear()
    ...
    if (len(plain_text) % 16) != 0:
        pad_count_input = 16 - len(plain_text) % 16
        plain_text = plain_text + " " * pad_count_input
    ...
    #convert cipher text to string, not bitvector
    cipher_text = cipher_text.get_bitvector_in_hex()
    #clear the roundkeys
    roundkeys.clear()
    return cipher_text
